...
# Fix for TF-IDF section too
num_docs = len(filenames)
K = min(3, num_docs - 1)  # Ensure K doesn't exceed available documents
...
for i, fname in enumerate(filenames):
    ...
    if K > 0:
        ...
        print(f"\nNo other documents to compare with '{fname}'")

# CORRECTED SEMANTIC SIMILARITY SECTION
...
# Step 3: Show top similar submissions for each document
# Fix: Ensure K doesn't exceed available documents
num_docs = len(filenames)
K = min(3, num_docs - 1)  # Ensure K doesn't exceed available comparisons (exclude self)

for i, fname in enumerate(filenames):
    # set self-similarity to -1 to ignore
    cosine_scores[i, i] = -1
    
    # Fix: Handle case where there are fewer documents than K
    if K > 0:  # Only proceed if there are documents to compare
...
code_extensions = ['.py', '.c', '.cpp', '.java', '.js']
code_files = [f for f in all_submissions.keys() if os.path.splitext(f)[1].lower() in code_extensions]

# Function to compute code similarity
def code_similarity(code_a, code_b):
    # Flatten code into strings (remove extra spaces)
    code_a = " ".join(code_a)
    code_b = " ".join(code_b)
    return difflib.SequenceMatcher(None, code_a, code_b).ratio()  # 0-1

# Compare all code files pairwise
K = 3
for i, fname in enumerate(code_files):
    sims = []
    for j, other in enumerate(code_files):
        if fname != other:
            score = code_similarity(all_submissions[fname], all_submissions[other])
            sims.append((score, other))
    sims.sort(key=lambda x: x[0], reverse=True)
    
    print(f"\nTop {K} code matches for '{fname}':")
    for s, other_file in sims[:K]:
        print(f"  {other_file} -> Code Similarity: {s:.2f}")
